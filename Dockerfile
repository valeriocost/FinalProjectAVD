...
# DOWNLOADS BASIC FOLDERS
RUN pip3 install gdown

# PythonAPI
RUN gdown 1Stdu9UrND97DN93LXc0_vvRUhnZ65DnZ
RUN tar -xf PythonAPI.tar
RUN mkdir /workspace/CARLA/
RUN mv /workspace/PythonAPI/ /workspace/CARLA/
RUN rm /workspace/PythonAPI.tar

# leaderboard_base
RUN gdown 1ypIsEM0QeyBrUtXjkWncl66bFFuJphsY
RUN tar -xf leaderboard_base.tar
RUN mv /workspace/carlaLeaderboard/* /workspace/
RUN rm -r /workspace/carlaLeaderboard/
RUN rm /workspace/leaderboard_base.tar


RUN pip3 install -r ${SCENARIO_RUNNER_ROOT}/requirements.txt
RUN pip3 install -r ${LEADERBOARD_ROOT}/requirements.txt

RUN mkdir /workspace/team_code
